    ...
    <footer className="bg-[#0b0b0d] text-slate-300">
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        <div>
          ...
          <div className="flex items-center gap-3 mt-6">
            {[Facebook, Instagram, Twitter, Youtube].map((Icon, i) => (
              <a key={i} href="#" className="w-9 h-9 rounded-full bg-white/5 hover:bg-yellow-500 flex items-center justify-center transition-colors">
                <Icon size={16} />
              </a>
            ))}
          </div>
        ...
        <div>
          ...
          <a href={`mailto:${brand.email}`} className="block text-white font-medium mt-1 hover:text-yellow-600 transition-colors">
      ...
      <button
        onClick={scrollTop}
        className="mx-auto -mb-5 relative z-10 w-10 h-10 rounded-full bg-yellow-400 hover:bg-yellow-500 flex items-center justify-center text-white shadow-lg block"
        aria-label="scroll top"
      >
        <ArrowUp size={18} />
      </button>

      <div className="bg-yellow-400 text-slate-900">
        <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col md:flex-row items-center justify-between gap-3 text-sm">
          ...
          <div className="flex items-center gap-5">
            ...
            <Link to="/refund" className="hover:text-slate-800">Refund Policy</Link>
